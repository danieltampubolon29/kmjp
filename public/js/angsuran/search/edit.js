document.addEventListener('DOMContentLoaded', function () {
    ...
    const angsuranId = "{{ $angsuran->id ?? '' }}"; // Ambil ID angsuran yang sedang diedit

    searchInput.addEventListener('input', function () {
        const query = this.value.trim();

        if (query.length > 0) {
            fetch(`/search-pencairan?q=${encodeURIComponent(query)}&angsuran_id=${angsuranId}`)
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(pencairan => {
                            ...
                            div.dataset.nominalPencairan = pencairan.nominal || 0;
