...
function showSimpananModal(title, simpananType) {
    ...
    fetch(
        `/get-simpanan-transactions?type=${encodeURIComponent(
            simpananType
        )}&anggota_id=${encodeURIComponent(anggotaId)}`
    )
        .then((response) => {
            if (!response.ok) {
        ...
        .then((data) => {
            ...
            if (data.length > 0) {
                ...
                tableBody.innerHTML =
                    '<tr><td colspan="4" class="text-center">Tidak ada data transaksi</td></tr>';
            }
        })
        .catch((error) => {
            console.error("Error fetching simpanan transactions:", error);
            tableBody.innerHTML =
                '<tr><td colspan="4" class="text-center">Terjadi kesalahan saat memuat data.</td></tr>';
        });
...
document.addEventListener("DOMContentLoaded", function () {
                    ...
                    if (data.length > 0) {
                        data.forEach((item) => {
                            ...
                            listItem.addEventListener("click", function () {
                                ...
                                const anggotaNama =
                                    listItem.getAttribute("data-nama");
                                const anggotaNo =
                                    listItem.getAttribute("data-no-anggota");
    ...
    function fetchSimpananData(anggotaId) {
        fetch(`/get-simpanan-data?anggota_id=${anggotaId}`)
            ...
            .then((data) => {
                if (data) {
                    document.getElementById("simpananPokok").textContent =
                        formatCurrency(data.pokok);
                    document.getElementById("simpananWajib").textContent =
                        formatCurrency(data.wajib);
                    document.getElementById("simpananSukarela").textContent =
                        formatCurrency(data.sukarela);
                    document.getElementById("simpananDeposito").textContent =
                        formatCurrency(data.deposito);
                } else {
